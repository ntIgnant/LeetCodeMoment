...
# Implement a DFS (depth first search) algorithm to find a the shortes path by number of nodes
...
def dfs(graph, source_node, goal_node):
    ...
    while frontier != []:
        ...
        # add the current last node to visited to avoid cyles
        ...
        for char, cost in last_node_succ:
            newPath = curr_path + char
            # add the path to the frontier (INPORTANT: Make it so it has STACK behavior bc DFS)
            frontier = [newPath] + frontier

    return f"Couldn't Get from node {source_node} to node {goal_node}"

test01 = dfs(glob_graph.given_graph, 'A', 'G')
print(test01)